...
type
  ...
  private
    ...
    function LastMatch(AState: Word; AOptions: Word): PView;
...
function GetSubViewPtr(var S: TStream; OwnerGroup: PGroup): PView;
...
begin
  if Last <> nil then begin
    ...
    while P <> nil do begin
      T := P^.Next;
      P^.Owner := nil;  { Prevent TView.Done from calling Delete }
      Dispose(P, Done);
      P := T;
    end;
  end;
...
function TGroup.LastMatch(AState: Word; AOptions: Word): PView;
var
  P: PView;
begin
...
procedure TGroup.SetCurrent(P: PView; Mode: SelectMode);
  ...
  procedure SelectView(V: PView; Enable: Boolean);
  begin
    if V <> nil then
      V^.SetState(sfSelected, Enable);
  end;

  procedure FocusView(V: PView; Enable: Boolean);
  begin
...
begin
  ...
  if (Owner <> nil) and (Width > 10) then begin
    TitleStr := PWindow(Owner)^.GetTitle(Width - 10);
...
var
  ...
  begin
    ...
    PWindow(Owner)^.SizeLimits(MinP, MaxP);
    Owner^.DragView(Event, PWindow(Owner)^.DragMode or Mode, Limits, MinP, MaxP);
    ClearEvent(Event);
  end;
...
const
  { Frame characters: 3 chars per row (left, middle, right)
    Row 0: inactive/drag top    (space, space, space)
    Row 1: inactive/drag side   (│, space, │) - single line
    Row 2: inactive/drag bottom (└, ─, ┘) - single line corners
    Row 3: active top           (╔, ═, ╗) - double line
    Row 4: active side          (║, space, ║) - double line
    Row 5: active bottom        (╚, ═, ╝) - double line }
...
constructor TListViewer.Init(var Bounds: TRect; ANumCols: Word; AHScrollBar, AVScrollBar: PScrollBar);
var
