...
begin inherited Init(Bounds); EventMask := EventMask or evBroadcast; end;
...
var AutoSelect, MouseActive: Boolean; Action: MenuAction; Ch: AnsiChar; Res: Word; R: TRect;
  ItemShown, P: PMenuItem; Target: PMenuView; E: TEvent;
...
begin
  ...
  repeat Action := DoNothing; GetEvent(E);
    case E.What of
    ...
    if (Action = DoSelect) or ((Action = DoNothing) and AutoSelect) then if Current <> nil then with Current^ do if Name <> nil then
      if Command = 0 then begin if E.What and (evMouseDown + evMouseMove) <> 0 then PutEvent(E);
        ...
        Target := TopMenu^.NewSubView(R, SubMenu, @Self); Res := Owner^.ExecView(Target); Dispose(Target, Done);
...
begin NewSubView := New(PMenuBox, Init(Bounds, AMenu, AParentMenu)); end;

procedure TMenuView.Store(var S: TStream); begin TView.Store(S); end;
...
begin inherited Init(Bounds); GrowMode := gfGrowHiX; Menu := AMenu; Options := Options or ofPreProcess; end;

destructor TMenuBar.Done; begin if Menu <> nil then DisposeMenu(Menu); inherited Done; end;
...
begin W := 0; H := 2; if AMenu <> nil then begin P := AMenu^.Items;
  ...
  inherited Init(R); State := State or sfShadow; Options := Options or ofFramed or ofPreProcess; Menu := AMenu; ParentMenu := AParentMenu; end;
...
constructor TMenuPopup.Init(var Bounds: TRect; AMenu: PMenu); begin inherited Init(Bounds, AMenu, nil); end;
destructor TMenuPopup.Done; begin if Menu <> nil then DisposeMenu(Menu); inherited Done; end;
...
begin inherited Init(Bounds); Options := Options or ofPreProcess; EventMask := EventMask or evBroadcast;
  GrowMode := gfGrowLoY + gfGrowHiX + gfGrowHiY; Defs := ADefs; FindItems; end;

constructor TStatusLine.Load(var S: TStream); begin inherited Load(S); Defs := nil; FindItems; end;
...
begin while Defs <> nil do begin T := Defs; Defs := Defs^.Next; DisposeItems(T^.Items); Dispose(T); end; inherited Done; end;
...
procedure TStatusLine.Update; var H: Word; P: PView;
...
procedure TStatusLine.Store(var S: TStream); begin TView.Store(S); end;
...
begin if (Name <> '') and (Command <> 0) then begin New(P); FillChar(P^, SizeOf(TMenuItem), 0);
  if P <> nil then begin P^.Next := Next; P^.Name := NewStr(Name); P^.Command := Command;
    R.Assign(1, 1, 10, 10); T := New(PView, Init(R)); if T <> nil then begin P^.Disabled := not T^.CommandEnabled(Command); Dispose(T, Done); end else P^.Disabled := True;
