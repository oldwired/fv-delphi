...
type
  ...
  TFileDialog = object(TDialog)
    ...
    WildCard: TWildStr;
    Directory: PString;
    constructor Init(AWildCard: TWildStr; const ATitle, InputName: String;
...
begin
  Finalize(PSearchRec(Item)^);  { Finalize managed fields before disposal }
...
begin
  New(Item);
  Initialize(Item^);  { Initialize managed fields }
...
begin
  { Clear visible state BEFORE disposing List to prevent DrawView during Hide }
...
begin
  ...
  begin
    ...
    begin
      New(P);
      Initialize(P^);  { Initialize managed fields before assignment }
      P^ := SR;
      FileList^.Insert(P);
    end;
    DosFindNext(SR);
...
begin
  TView.Init(Bounds);
  Initialize(S);  { Properly initialize managed fields }
  S.Attr := 0;
  S.Time := 0;
  S.Size := 0;
  S.Name := '';
...
destructor TFileInfoPane.Done;
begin
  Finalize(S);  { Finalize managed fields before destruction }
  inherited Done;
end;
...
begin
  ...
  { Convert string Name to ShortString for FormatStr }
...
constructor TFileHistory.Init(var Bounds: TRect; ALink: PInputLine; AHistoryId: Word);
begin
...
constructor TFileDialog.Init(AWildCard: TWildStr; const ATitle, InputName: String;
  AOptions: Word; HistoryId: Byte);
var
  Control: PView;
  R: TRect;
...
begin
  ...
  FileName := PFileInputLine(GetSubViewPtr(S, @Self));
  FileList := PFileList(GetSubViewPtr(S, @Self));
  FileHistory := PFileHistory(GetSubViewPtr(S, @Self));
  ReadDirectory;
end;
...
begin
  ...
  Inc(R.A.Y, 3);
  Inc(R.B.Y, 3);
  ChDirButton := New(PButton, Init(R, slChDir, cmChangeDir, bfNormal));
  Insert(ChDirButton);
  Inc(R.A.Y, 3);
  Inc(R.B.Y, 3);
...
begin
  TDialog.Load(S);
  DirList := PDirListBox(GetSubViewPtr(S, @Self));
  DirInput := PInputLine(GetSubViewPtr(S, @Self));
  OkButton := PButton(GetSubViewPtr(S, @Self));
  ChDirButton := PButton(GetSubViewPtr(S, @Self));
  SetUpDialog;
end;
...
var
  Dlg: PFileDialog;
begin
  Dlg := New(PFileDialog, Init('*.*', sOpen, slName, fdOkButton or fdHelpButton, 0));
...
begin
  Result := False;
  Dlg := New(PFileDialog, Init('*.*', sSaveAs, slSaveAs, fdOkButton or fdHelpButton, 0));
  PHistory(Dlg^.FileName^.Next^.Next)^.HistoryID := HistoryID;
...
finalization
  { Clean up managed field in static buffer }
