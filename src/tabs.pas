...
function NewTabDef(const AName: ShortString; ADefItem: PView; AItems: PTabItem; ANext: PTabDef): PTabDef;
...
begin
  ...
  begin
    ...
    begin
      DefCount := 1;
      while AtTab(DefCount - 1)^.Next <> nil do
        Inc(DefCount);
    end
    else
      DefCount := 0;
...
var
  ...
  I: SmallInt;
begin
  if (Index < 0) or (Index >= DefCount) or (DefCount <= 1) then
    Exit;

  { If removing active tab, switch to another first }
  if Index = ActiveDef then begin
    if Index > 0 then
      SelectTab(Index - 1)
    else
      SelectTab(Index + 1);
      ...
      kbTab, kbShiftTab:
        ...
        begin
          ...
          begin
            ...
            if ((Current = LastV) or (Current = PLabel(LastV)^.Link)) and
               (Event.KeyCode = kbShiftTab) then
            begin
              if Owner <> nil then Owner^.SelectNext(True);
            end
            else if ((Current = FirstV) or (Current = PLabel(FirstV)^.Link)) and
